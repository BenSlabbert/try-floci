...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        String opensearchEndpoint = envOrDefault("OPENSEARCH_ENDPOINT", "http://localhost:9200");
        String indexName = envOrDefault("OPENSEARCH_INDEX", "events");
        ...
        if (accessKey != null && secretKey != null) {
            kinesisBuilder.credentialsProvider(
        ...
        vertx.deployVerticle(new ConsumerVerticle(kinesisClient, streamName, opensearchEndpoint, indexName))
                .onSuccess(id -> log.info("OpenSearch consumer deployed: {}", id))
                .onFailure(err -> {
                    log.error("Failed to deploy OpenSearch consumer", err);
                    vertx.close();
                });
